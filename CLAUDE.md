...
6. **package** — `cli._build_one` branches on the target's `format`:
   ...
   - `"image"` (`archive.py`, **any platform**): no installer — the image tree itself is archived into `<target>/dist/<name>-<version>-<target>.zip` (Windows targets) or `.tar.gz` (Linux/macOS targets; reuses the posix tar machinery so symlinks/ownership are handled), under a top-level `<name>-<version>/` dir. Launchers follow the target OS (MSVC `.exe` via `build_launchers` — signed only when `PYAPPDIST_SIGN_CMD` is set — or the posix shell wrappers written by `archive.build_archive`); a target-level `no-launcher = true` (image-only, else `ConfigError`) skips launchers entirely.
