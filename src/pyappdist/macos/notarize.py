...
After Developer ID signing (:mod:`.sign`), the artifact is submitted to Apple's
notary service and, once accepted, the ticket is stapled so it validates offline.
...
def _submit(artifact: Path, profile: str, *, log) -> None:
    ...
    # --wait exits 0 once a terminal status is reached; require an Accepted status.
    if proc.returncode != 0 or "status: Accepted" not in proc.stdout:
        raise BuildError(
            f"notarization failed for {artifact.name}:\n{proc.stdout}\n{proc.stderr}\n"
            "Inspect the log with: xcrun notarytool log <submission-id> "
