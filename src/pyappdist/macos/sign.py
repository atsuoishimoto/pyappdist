...
* **Developer ID** — when ``signing_identity`` (or ``PYAPPDIST_SIGNING_IDENTITY``) is set:
  ...
  entitlements (a bundled-python default unless ``entitlements`` overrides it). This is the
  signature notarization requires (see :mod:`.notarize`).
"""
...
# Hardened-runtime entitlements a bundled CPython needs to load its own (and third-party)
# extension modules. Intentionally permissive ("get it signing, narrow later"); the most
# important one is disable-library-validation, which lets the hardened interpreter load
# .so/.dylib files signed by a different identity (or ad-hoc).
