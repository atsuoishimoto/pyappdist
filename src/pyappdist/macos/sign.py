...
* **Developer ID** — when ``PYAPPDIST_MACOS_SIGNING_IDENTITY`` (env, takes precedence)
  or ``signing-identity`` (config) is set:
...
_IDENTITY_ENV = "PYAPPDIST_MACOS_SIGNING_IDENTITY"
...
def resolve_sign_options(config: Config, build_dir: Path, *, log=print) -> SignOptions:
    ...
    ``PYAPPDIST_MACOS_SIGNING_IDENTITY`` (env, overriding ``signing-identity``) or
    ``signing-identity`` selects Developer ID, which
    ...
    """
    identity = os.environ.get(_IDENTITY_ENV) or config.macos.signing_identity
    if not identity:
        log("macos: signing ad-hoc (set signing-identity / PYAPPDIST_MACOS_SIGNING_IDENTITY for Developer ID)")
        return SignOptions()
