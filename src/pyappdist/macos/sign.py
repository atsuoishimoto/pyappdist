...
* **Developer ID** — when ``signing-identity`` (or ``PYAPPDIST_MACOS_SIGNING_IDENTITY``) is set:
...
def resolve_sign_options(config: Config, build_dir: Path, *, log=print) -> SignOptions:
    ...
    ``signing-identity`` (or ``PYAPPDIST_MACOS_SIGNING_IDENTITY``) selects Developer ID, which
