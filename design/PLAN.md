...
## 確定方針
- ターゲット: Windows x64（MVP）。依存は wheel を優先しつつ sdist も許容（事前拒否しない）。macOS/Linux・自動アップデータ・MSIX・複雑な CustomAction は対象外。
...
wix/
  ...
  guid.py         # 安定 GUID = uuid5(upgrade-code, install相対パス)
...
- 完了: `pyappdist build-wheels sample/helloworld` で `appdist/wheelhouse/*.whl`(app+deps) 生成。`fetch-runtime` で `appdist/runtime/` へ展開・検証。
...
- **stable component GUID**: uuid5(upgrade-code, 相対パス) で決定的生成。Component=1 File 原則で KeyPath 単純化。
- **sdist の扱い**: sdist も許容（事前拒否しない）。pure-Python sdist は cross-build でも `py3-none-any` wheel になり問題なし。C 拡張で Windows wheel が無く sdist のみの依存は cross-build で自然に失敗する（事前判定はしない）→ ローカル Windows 実機(`uv.exe`)なら build 可能なケースもある。
